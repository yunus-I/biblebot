...
// Replace with your actual Firebase Service Account Key JSON file path
...
// Change: Support multiple admins using a comma-separated list in .env
// Example in .env: ADMIN_CHAT_IDS=123456789,987654321
const ADMIN_CHAT_IDS = process.env.ADMIN_CHAT_IDS 
  ? process.env.ADMIN_CHAT_IDS.split(',').map(id => id.trim())
  : ['YOUR_ADMIN_TELEGRAM_ID_1', 'YOUR_ADMIN_TELEGRAM_ID_2']; 
...
// ==========================================
// 2. BILINGUAL MESSAGES (No language selection needed)
// ==========================================
const msgText = {
...
bot.onText(/\/start/, (msg) => {
  ...
  // Initialize user session and capture Telegram details immediately
  userSessions[chatId] = { 
    step: 'ASK_NAME',
...
// Handle text inputs based on user session state
bot.on('message', async (msg) => {
  const chatId = msg.chat.id;
  const text = msg.text;
  const session = userSessions[chatId];

  // Ignore commands or messages without a session or if a photo is expected
...
// Handle photo uploads (Payment Receipt)
bot.on('photo', async (msg) => {
  ...
  try {
    // 1. Save user to Firestore with isPaid: false
    ...
    await userRef.set({
      ...
      isPaid: false,
      deviceId: null, // Ready for app login binding
      registeredAt: admin.firestore.FieldValue.serverTimestamp()
    });

    // 2. Notify User to wait
    bot.sendMessage(chatId, msgText.wait);

    // 3. Send data and photo to ALL Admins
    const adminMessage = `
...
💬 **TG Username:** ${session.username}
    ...
    // Broadcast to all admins listed in the environment variables
...
// ==========================================
// 4. INLINE BUTTON CALLBACKS (Admin Controls)
// ==========================================
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('approve_') || data.startsWith('reject_')) {
    ...
    // Ensure only listed admins can click these buttons
    ...
    // Get the name/username of the admin who clicked the button
    ...
    try {
      ...
      if (action === 'approve') {
        ...
        // Update Admin Message (Keep buttons, add Approved By text)
        bot.editMessageCaption(`${query.message.caption}\n\n✅ **Approved by ${adminIdentity}**`, {
          chat_id: chatId,
          message_id: messageId,
          parse_mode: 'Markdown',
          reply_markup: query.message.reply_markup // Keeps the buttons intact
        });
        ...
        // Update Admin Message (Keep buttons, add Rejected By text)
...
console.log('🤖 Bible App Registration Bot is running...');